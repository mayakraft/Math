...
test("convexHull", () => {
	...
	const res0 = math.convexHull(rect)
		.map(v => rect[v]);
	const res1 = math.convexHull(rect, true)
		.map(v => rect[v]);
...
test("convexHull collinear", () => {
	...
	const res0 = math.convexHull(rect_collinear)
		.map(v => rect_collinear[v]);
	const res1 = math.convexHull(rect_collinear, true)
		.map(v => rect_collinear[v]);
	expect(res0.length).toBe(4);
