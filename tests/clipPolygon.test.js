...
test("clipPolygonPolygon edge adjacent non intersecting", () => {
	...
	const res1 = math.clipPolygonPolygon(poly1, poly2);
	const res2 = math.clipPolygonPolygon(poly1, poly2, 0.1);
	const res3 = math.clipPolygonPolygon(poly1, poly2, -0.1);
...
test("clipPolygonPolygon enclosing polygons", () => {
	...
	const res1 = math.clipPolygonPolygon(poly1, poly2);
	const res2 = math.clipPolygonPolygon(poly2, poly1);
...
test("clipPolygonPolygon same vertex, edge on vertex", () => {
	...
	const res2 = math.clipPolygonPolygon(poly1, poly3);
	testEqualVectorVectors(
		res2,
		[[1, 0], [1, 1], [0, 1]],
	);
...
test("clipPolygonPolygon ensure input parameters did not modify", () => {
	const poly1 = [[2, 0], [0, 2], [-2, 0], [0, -2]];
	const poly2 = [[1, -1], [3, 1], [1, 3], [-1, 1]];
	math.clipPolygonPolygon(poly1, poly2);
	math.clipPolygonPolygon(poly2, poly1);
	testEqualVectorVectors(
		poly1,
		[[2, 0], [0, 2], [-2, 0], [0, -2]],
	);
