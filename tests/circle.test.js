...
test("arguments", () => {
	...
	expect(math.circle([1,2], [3,4]).radius).toBe(math.core.distance2([1,2], [3,4]));
	expect(math.circle([1,2], [3,4]).origin.x).toBe(1);
	expect(math.circle([1,2], [3,4]).origin.y).toBe(2);
...
test("intersect lines", () => {
	...
	math.core.fnEpsilonEqualVectors(clipLine[0], shouldBeLine[0]);
	math.core.fnEpsilonEqualVectors(clipLine[1], shouldBeLine[1]);
	// no intersect
	...
	math.core.fnEpsilonEqualVectors(shouldBeRay, clipRay[0]);

	const shouldBeSeg = [Math.sqrt(2) / 2, Math.sqrt(2) / 2];
	const clipSeg = math.circle(1).intersect(math.segment(0, 0, 10, 10));
	math.core.fnEpsilonEqualVectors(shouldBeSeg, clipSeg[0]);
});
