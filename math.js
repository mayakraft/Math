...
(function (global, factory) {
  ...
  const circle_circle = function (center, radius, center2, radius2, epsilon = EPSILON) {
  ...
  const circle_line = function (center, radius, lpt, lvec, epsilon = EPSILON) {
  ...
  const circle_ray = function (center, radius, lpt, lvec, epsilon = EPSILON) {
    const magSq = lvec[0] ** 2 + lvec[1] ** 2;
    const mag = Math.sqrt(magSq);
    const norm = mag === 0 ? lvec : lvec.map(c => c / mag);
    const rot90 = [-norm[1], norm[0]];
  ...
  const circle_segment = function (center, radius, p1, p2, epsilon = EPSILON) {
    const lpt = p1;
  ...
  const vector = function () {
    ...
    Object.keys(M).forEach(key => Object.defineProperty(v, key, {
      value: M[key].bind(v)
    }));
  ...
  const map = {
    circle: {
      poly: (a,b) => convex_poly_circle(b, a.origin, a.radius),
      circle: (a,b) => circle_circle(a.origin, a.radius, b.origin, b.radius),
      line: (a,b) => circle_line(a.origin, a.radius, b.origin, b.vector),
      ray: (a,b) => circle_ray(a.origin, a.radius, b.origin, b.vector),
      segment: (a,b) => circle_segment(a.origin, a.radius, b[0], b[1]),
    },
    line: {
      poly: (a,b) => convex_poly_line(b, a.origin, a.vector),
      circle: (a,b) => circle_line(b.origin, b.radius, a.origin, a.vector),
      line: (a,b) => line_line(a.origin, a.vector, b.origin, b.vector),
      ray: (a,b) => line_ray(a.origin, a.vector, b.origin, b.vector),
      segment: (a,b) => line_segment(a.origin, a.vector, b[0], b[1]),
    },
    ray: {
      poly: (a,b) => convex_poly_ray(b, a.origin, a.vector),
      circle: (a,b) => circle_ray(b.origin, b.radius, a.origin, a.vector),
      line: (a,b) => line_ray(b.origin, b.vector, a.origin, a.vector),
      ray: (a,b) => ray_ray(a.origin, a.vector, b.origin, b.vector),
      segment: (a,b) => ray_segment(a.origin, a.vector, b[0], b[1]),
    },
    segment: {
      poly: (a,b) => convex_poly_segment(b, a.origin, a.vector),
      circle: (a,b) => circle_segment(b.origin, b.radius, a[0], a[1]),
      line: (a,b) => line_segment(b.origin, b.vector, a[0], a[1]),
      ray: (a,b) => ray_segment(b.origin, b.vector, a[0], a[1]),
      segment: (a,b) => segment_segment(a[0], a[1], b[0], b[1]),
    },
  };
  ...
  function Prototype (subtype, prototype) {
    ...
    const intersect$1 = function (other) {
    ...
    Object.defineProperty(proto, "intersect", { value: intersect$1 });
  ...
  const Line = function (...args) {
    const { origin, vector: vector$1 } = get_line(args);
    const transform = function (...innerArgs) {
      const mat = get_matrix2(innerArgs);
      const line = multiply_matrix2_line2(mat, origin, vector$1);
  ...
  const Ray = function (...args) {
    ...
    const transform = function (...innerArgs) {
      const mat = get_matrix2(innerArgs);
      const vec_translated = vector$1.map((vec, i) => vec + origin[i]);
    ...
    Object.defineProperty(ray, "vector", { get: () => vector(vector$1) });
  ...
  const Segment = function (...args) {
    ...
    Object.defineProperty(segment, "vector", { get: () => vector$1() });
